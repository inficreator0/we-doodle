  ...
  useEffect(() => {
    ...
    socket.on('board-removed', (boardName: string) => {
      ...
      setBoards((prevBoards) =>
        prevBoards.filter((board) => board.name !== boardName)
      )
    })
        ...
        <div className="bg-white/5 backdrop-blur-sm rounded-xl p-8 mb-8 shadow-xl border border-white/10">
          ...
          <form onSubmit={handleCreateBoard} className="flex gap-4">
            ...
            <button
              type="submit"
              disabled={isCreating}
              className={`px-6 py-3 bg-teal-500 text-white rounded-lg hover:bg-teal-600 transition-all duration-300 flex items-center gap-2 ${
        ...
        <div className="bg-white/5 backdrop-blur-sm rounded-xl p-8 shadow-xl border border-white/10">
          ...
          {loading ? (
            ...
            <div className="text-center py-12 text-white/70">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="h-16 w-16 mx-auto mb-4 text-white/50"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
