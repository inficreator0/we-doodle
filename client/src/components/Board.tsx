...
const Board = () => {
  ...
  useEffect(() => {
    if (state.isJoined && boardName) {
      ...
      socket.on('user-color-update', ({ username, color }: { username: string; color: string }) => {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-5 bg-gradient-to-br from-gray-900 to-gray-800 relative overflow-hidden">
        ...
        <div className="relative z-10 w-full max-w-md">
          <h1 className="text-4xl font-bold text-white mb-8 text-center">Join Board</h1>
          <form onSubmit={handleJoin} className="w-full bg-white/5 backdrop-blur-sm rounded-xl p-8 shadow-xl border border-white/10">
            <input
              type="text"
